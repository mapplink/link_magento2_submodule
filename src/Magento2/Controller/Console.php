<?php
/*
 * Module Test > ToDo: Move the a proper unit test
 * @category Magento2
 * @package Magento2\Controller
 * @author Andreas Gerhards <andreas@lero9.co.nz>
 * @copyright Copyright (c) 2016 LERO9 Ltd.
 * @license Commercial - All Rights Reserved
 */

namespace Magento2\Controller;
...
class Console extends AbstractConsole
{
    ...
    protected function eavtestTask($nid)
    {
        ...
        if (!$nodeEntity) {
            throw new MagelinkException('Could not find Magento2 node');
        }

        $node = new \Magento2\Node();
        if ($node instanceof ServiceLocatorAwareInterface) {
            $node->setServiceLocator($this->getServiceLocator());
        }
        $node->init($nodeEntity);

        /** @var \Magento2\Api\Db $db */
        $db = $this->getServiceLocator()->get('magento2_db');
        if (!$db->init($node)) {
    ...
    protected function testorderTask($nid)
    {
        ...
        $restV1 = $this->getServiceLocator()->get('magento2_restV1');
        if (!$restV1->init($node)) {
        ...
        $query = 'SELECT p.unique_id AS k, si.available AS v FROM {product:p:visible,enabled,type}'
            .' JOIN {stockitem:si:available,pickable} ON si.parent_id = p.entity_id'
            .' WHERE si.available > 1 AND p.visible = 1 AND p.enabled = 1 AND p.type = "simple"';
        $availableProducts = $entityService->executeQueryAssoc($query);
        ...
        for($i = 0; $i < $orderCount; $i++) {
            ...
            if (!$preserveCart) {
                ...
                // Set up new cart
                $cid = $restV1->call('shoppingCartCreate', array(1));
                if (!intval($cid)) {
                    throw new MagelinkException('Failed to create shopping cart');
                }

                $restV1->call('shoppingCartCustomerSet', array($cid, $customerData, 1));
                $restV1->call('shoppingCartCustomerAddresses', array($cid, $addressData, 1));
            }
            ...
            try{
                $restV1->call('shoppingCartProductAdd', array($cid, $toAdd, 1));
            }catch(\Exception $e) {
            ...
            $restV1->call('shoppingCartShippingMethod', array($cid, $shippingMethod, 1));

            $res = $restV1->call('shoppingCartTotals', array($cid, 1));
            var_export($res);
            echo PHP_EOL;

            $restV1->call('shoppingCartPaymentMethod', array($cid, $paymentData, 1));

            echo 'Placing order...'.PHP_EOL;
            $orderId = $restV1->call('shoppingCartOrder', array($cid, 1));
            ...
            if ($invoice) {
                ...
                $orderData = $restV1->call('salesOrderInfo', array($orderId));
                ...
                $restV1->call('salesOrderInvoiceCreate', array($orderId, $itemIds));
            }
